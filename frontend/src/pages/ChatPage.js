  ...
  return (
    <div className="min-h-screen py-6">
      <div className="section">
        <h1 className="text-2xl font-display font-bold text-surface-900 mb-6">Messages</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 h-[calc(100vh-200px)]">
          ...
          <div className="lg:col-span-1 card overflow-hidden flex flex-col">
            <div className="p-4 border-b border-surface-900/8">
              <h2 className="text-sm font-display font-semibold text-surface-900">Conversations</h2>
            </div>
            <div className="flex-1 overflow-y-auto">
              {loadingConvos ? (
                ...
                <div className="text-center py-10 px-4">
                  <FiMessageCircle size={28} className="mx-auto mb-2 text-surface-700/30" />
                  <p className="text-surface-700 text-sm">No conversations yet</p>
                  <p className="text-surface-700/60 text-xs mt-1">Hire a service to start chatting</p>
                </div>
              ) : (
                conversations.map((convo) => (
                  <button key={convo.room}
                    onClick={() => openChat(convo.otherUser?._id, convo.otherUser)}
                    className={`w-full flex items-center gap-3 p-4 text-left transition-colors border-b border-surface-900/5 hover:bg-surface-200/50
                      ${activeRoom === convo.room ? 'bg-surface-200 border-l-2 border-l-brand-500' : ''}`}>
                    {/* Avatar */}
                    <div className="relative shrink-0">
                      {convo.otherUser?.profileImage ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-brand-500/15 flex items-center justify-center">
                          <span className="text-brand-500 font-semibold">{convo.otherUser?.name?.[0]}</span>
                        </div>
                      )}
                      {isUserOnline(convo.otherUser?._id) && (
                        <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 rounded-full border-2 border-surface-200" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-surface-900 text-sm font-medium truncate">{convo.otherUser?.name}</span>
                        <span className="text-surface-700 text-xs shrink-0">
                          {formatTime(convo.lastMessage?.createdAt)}
                        </span>
                      </div>
                      <p className="text-surface-700 text-xs truncate mt-0.5">{convo.lastMessage?.content}</p>
                    </div>
                    {convo.unreadCount > 0 && (
          ...
          <div className="lg:col-span-2 card flex flex-col overflow-hidden">
            {!activeUser ? (
              <div className="flex-1 flex flex-col items-center justify-center text-surface-700 gap-3">
                <FiMessageCircle size={40} className="opacity-20" />
                <p className="font-medium">Select a conversation</p>
                <p className="text-sm text-surface-700/60">Or hire a service to start chatting</p>
              </div>
            ) : (
              <>
                {/* Chat header */}
                <div className="flex items-center gap-3 px-5 py-4 border-b border-surface-900/8">
                  ...
                  <div>
                    <Link to={`/profile/${activeUser?._id}`}
                      className="text-surface-900 font-medium text-sm hover:text-brand-500 transition-colors">
                      {activeUser?.name}
                    </Link>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-surface-50">
                        ...
                        <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-xs lg:max-w-sm px-4 py-2.5 rounded-2xl ${
                            isMe
                              ...
                              : 'bg-surface-200 text-surface-900 rounded-bl-sm border border-surface-900/8'
                          }`}>
                            <p className="text-sm leading-relaxed">{msg.content}</p>
                            <p className={`text-xs mt-1 ${isMe ? 'text-white/60' : 'text-surface-700'}`}>
                  ...
                  {isTyping && (
                    <div className="flex justify-start">
                      <div className="bg-surface-200 px-4 py-2.5 rounded-2xl rounded-bl-sm border border-surface-900/8">
                        <div className="flex gap-1">
                          {[0, 1, 2].map((i) => (
                            <span key={i} className="w-2 h-2 bg-surface-700 rounded-full animate-bounce"
                ...
                {/* Input */}
                <form onSubmit={sendMessage} className="flex items-center gap-3 p-4 border-t border-surface-900/8">
