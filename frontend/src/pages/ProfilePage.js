            ...
            <div className="card p-6">
              ...
              <div className="relative w-24 h-24 mx-auto mb-4">
                {avatarUrl ? (
                  ...
                  <div className="w-24 h-24 rounded-full bg-brand-500/20 flex items-center justify-center ring-4 ring-brand-500/30">
                    <span className="text-brand-400 font-bold text-3xl">{profile.name?.[0]?.toUpperCase()}</span>
                  </div>
                )}
                {editing && (
                  <label className="absolute inset-0 rounded-full bg-black/60 flex items-center justify-center cursor-pointer hover:bg-black/70 transition-colors">
              ...
              {editing ? (
                ...
                <div className="text-center mb-4">
                  ...
                  {profile.college && <p className="text-slate-400 text-sm mt-2">{profile.college}</p>}
                </div>
              )}

              {/* Rating */}
              {profile.averageRating > 0 && (
                <div className="flex items-center justify-center gap-2 mb-4 pb-4 border-b border-white/8">
                  <StarDisplay rating={profile.averageRating} size={14} />
                  <span className="text-amber-400 font-semibold text-sm">{profile.averageRating.toFixed(1)}</span>
                  <span className="text-slate-500 text-xs">({profile.totalReviews})</span>
                </div>
              )}

              {/* Stats */}
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="text-center p-3 bg-surface-900 rounded-xl">
                  <FiPackage size={18} className="text-brand-400 mx-auto mb-1" />
                  <p className="text-white font-bold">{services.length}</p>
                  <p className="text-slate-500 text-xs">Services</p>
                </div>
                <div className="text-center p-3 bg-surface-900 rounded-xl">
                  <FiStar size={18} className="text-amber-400 mx-auto mb-1" />
                  <p className="text-white font-bold">{profile.totalReviews || 0}</p>
              ...
              {editing ? (
                ...
                <>
                  {profile.bio && <p className="text-slate-300 text-sm leading-relaxed mb-4">{profile.bio}</p>}
                    ...
                    <div key={review._id} className="card p-5">
                      <div className="flex items-start gap-3 mb-2">
                        {review.reviewer?.profileImage
                          ? <img src={review.reviewer.profileImage} alt={review.reviewer.name} className="w-9 h-9 rounded-full object-cover shrink-0" />
                        ...
                        <div className="flex-1">
                          ...
                          {review.service && (
                            <Link to={`/services/${review.service._id}`} className="text-brand-400 text-xs hover:underline mt-0.5 block">
